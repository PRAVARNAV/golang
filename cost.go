...
const (
	...
	client_id = " fWy8Q~t-Y5m~IgG1z-UouC51c~A~S5YpiT0cTaMS"
...
/*creating table for azure billing*/
	...
	_, err = db.Exec("CREATE TABLE  IF  NOT EXISTS azure_billing_new_cost(id VARCHAR(300),Resource_id  character varying(400), Resource_name varchar(300),Resource_group varchar(300),tendenid varchar(300) ,consumedService VARCHAR(300), subscriptionid VARCHAR(300),BillingCurrency  VARCHAR(300),usageQuantity   NUMERIC,meterId VARCHAR(300), BillingPeriodStartDate VARCHAR(300) , BillingPeriodEndDate VARCHAR(300),UnitPrice float ,Location varchar(40),SkuId varchar(40),ServiceName varchar(40),UnitOfMeasure VARCHAR(40),ArmSkuName VARCHAR(40),Cost NUMERIC );")
	checkError(err)
...
for i, p := range response.Consumptions {
    ...
    int1, err := strconv.ParseInt(str1[0:4], 0, 0)
    int2, err := strconv.ParseInt(str1[6:7], 0, 0)
    int3, err := strconv.ParseInt(str1[9:10], 0, 0)

	str2 := p.Properties.BillingPeriodEndDate
    // using ParseInt method
    int4, err := strconv.ParseInt(str2[0:4], 0, 0)
    int5, err := strconv.ParseInt(str2[6:7], 0, 0)
    int6, err := strconv.ParseInt(str2[9:10], 0, 0)

// date_difference1 := int1 - int4
// date_difference2 := int2 - int5
// date_difference3 := int3 - int6
//________________________________________________________________________________________________________________________________
	
        start := time.Date(int(int1),time.Month(int2),int(int3),0, 00, 0, 0, time.UTC)
// start := (time.Date(p.Properties.BillingPeriodStartDate) , time.UTC)
	// fmt.Println(start)
	// fmt.Println(time.Now())
    end := time.Date(int(int4),time.Month(int5),int(int6),0, 00, 0, 0, time.UTC)
	
        // calculate years, month, days and time betwen dates
        year, month, day, hour, min, sec := diff(start, end)

// months_diff := ((year * 12) + (month * 1) )
// fmt.Println(months_diff,"#$%%$#############################33333333333333333")

fmt.Println(start , "start date============ ", end , "end date ========================")
...
/* Here we are inserting into the azure billing table*/
	_, err = db.Exec("INSERT INTO azure_billing_new_cost(Id,Resource_id,Resource_name,Resource_group,tendenid,ConsumedService ,Subscriptionid , BillingCurrency ,UsageQuantity ,MeterId , BillingPeriodStartDate , BillingPeriodEndDate) VALUES ($1, (Lower($2)), $3, (Lower($4)),$5, $6, (Lower($7)), $8,$9, (Lower($10)),$11,$12) ",
...
for key, value := range n {
 ...
 sqlStatements :=
	...
	_, err = db.Exec(sqlStatements, key,value)
	if err != nil {  
		panic(err)
	}	
}
