...
for i, k := range pro_dev_p {
	...
	if k == strings.ToLower(p.Properties.ResourceId) {
		...
		// m[k] += append(pro_devps,p.Properties.BillingPeriodStartDate) 
		m[k] = []string{p.Properties.BillingPeriodStartDate}
		m[k] = append(m[k],p.Properties.BillingPeriodStartDate )
